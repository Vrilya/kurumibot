...
module.exports = client => {
  client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.isSelectMenu()) {
      let link = "https://open.spotify.com/playlist/37i9dQZF1DWU1f2CQHNexG?si=6786caf254164f8b";
      if (interaction.values[0]) {
        //K-Pop 2024
        if (interaction.values[0].toLowerCase().startsWith("k")) link = "https://open.spotify.com/playlist/2EoheVFjqIxgJMb8VnDRtZ";
        //Aggrotech, Harsh EBM, Industrial
        if (interaction.values[0].toLowerCase().startsWith("agg")) link = "https://open.spotify.com/playlist/1PsMQlUhGksx5OBKO8Anpz";
        //default
        if (interaction.values[0].toLowerCase().startsWith("d")) link = "https://open.spotify.com/playlist/2YG4fdSRohmAQxW1zE2fZN";
        //Midsummer Solstice
        if (interaction.values[0].toLowerCase().startsWith("mid")) link = "https://open.spotify.com/playlist/2xWMsuuZRIHbj5bZOG6Cpm"
        //Synthmix
        if (interaction.values[0].toLowerCase().startsWith("syn")) link = "https://open.spotify.com/playlist/37i9dQZF1EIhx14vjEjMgT";
        //oldgaming
        ...
        //Best of Darkwave
        if (interaction.values[0].toLowerCase().startsWith("bes")) link = "https://open.spotify.com/playlist/5jymRcVFR42T9DbQMOT5jR";
        //Orgasmic
        //if (interaction.values[0].toLowerCase().startsWith("org")) link = "https://open.spotify.com/playlist/1UBNCp6aLkZNzvRJ9Jhwan"
        //Orgasmic
        if (interaction.values[0].toLowerCase().startsWith("org")) link = "https://open.spotify.com/playlist/1UBNCp6aLkZNzvRJ9Jhwan"
        //Retro Synthwave
        if (interaction.values[0].toLowerCase().startsWith("ret")) link = "https://open.spotify.com/playlist/3ebHKSjHujS4Tyt2KKP97R";
        //Dark Synthwave
        if (interaction.values[0].toLowerCase().startsWith("dark")) link = "https://open.spotify.com/playlist/3gmpLUPjxqgEAyeT793OpE";
        //dark synthpop mix
        if (interaction.values[0].toLowerCase().startsWith("dar")) link = "https://open.spotify.com/playlist/37i9dQZF1EIgszvYrzhf9n";
        //magic-release
        ...
        //J-Pop, J-Rock, Anime
        if (interaction.values[0].toLowerCase().startsWith("j")) link = "https://open.spotify.com/playlist/5SY8V1uXPByFVrkJFNdTJG";
        //heavy metal
  ...
  client.on("messageCreate", async message => {
    ...
    if (message.author.id === client.user.id) {
      ...
      message.delete().catch((e) => {
        console.log("Kunde inte radera meddelandet, det kan redan ha tagits bort.", e);
      });
    } else {
      message.delete().catch((e) => {
        console.log("Kunde inte radera meddelandet, det kan redan ha tagits bort.", e);
...
function generateQueueEmbed(client, guildId, leave) {
  ...
  var embeds = [
    ...
    .setImage(guild.banner ? guild.bannerURL({
      size: 4096
    }) : "https://i.imgur.com/AZ6320T.png")
    .setTitle(`Start Listening to Music, by connecting to a Voice Channel and sending either the **SONG LINK** or **SONG NAME** in this Channel!`)
    .setDescription(`> *Supports Youtube, Soundcloud, and direct MP3 Links!*`)
  ]
  ...
  var musicmixMenu = new MessageSelectMenu()
    ...
    .addOptions(["Aggro, EBM, Indust", "Dark Synthpop Mix", "Best of Darkwave", "Orgasmic", "Synthmix", "Retro Synthwave", "Dark Synthwave", "J-Pop, J-Rock, Anime", "Midsummer Solstice", "K-Pop 2024", "Default"].map((t, index) => {
      return {
        label: t.substr(0, 25),
        value: t.substr(0, 25),
