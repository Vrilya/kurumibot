  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    let tempmsg = await message.reply({
      embeds: [new Discord.MessageEmbed().setColor(es.color)
        .setAuthor({ name: client.la[ls].cmds.info.botinfo.loading, iconURL: "https://cdn.discordapp.com/emojis/756773010123522058.gif", url: "https://discord.gg/yNCuFxU8" })
      ]
    })
    cpuStat.usagePercent(function(e, percent, seconds) {
      ...
      const botinfo = new Discord.MessageEmbed()
        .setAuthor({ name: client.user.tag + " Information", iconURL: es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL(), url: `https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands` })
